...
#
#修改默认IP
sed -i ' s/192.168.1.1/10.0.5.1/g '包/基本文件/文件/bin/config_generate
# sed -i 's/default "5"/default "0"/g' config/Config-images.in
# sed -i 's/ipad=${ipaddr:-"192.168.$((addr_offset++)).1"}/ipad=${ipaddr:-"10.0.$((addr_offset++)).1"}/g ' 包/基础文件/文件/bin/config_generate
...
# sed -i 's/ OpenWrt/Sunvid /g' package/base-files/files/bin/config_generate
# 修改启动0秒
sed -i ' s/默认“5”/默认“0” /g '  config/Config-images.in

#修改版本号
# sed -i "s/OpenWrt /P3TERX build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/lean/default-settings/files/zzz-default-设置
#修改掩码
sed -i ' s/255.255.255.0/255.255.248.0/g '包/基本文件/文件/bin/config_generate
